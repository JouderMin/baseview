...
use raw_window_handle::{
    ...
    HasRawDisplayHandle, HasRawWindowHandle, HasWindowHandle, RawDisplayHandle, RawWindowHandle,
};
...
impl WindowInner {
    ...
    fn raw_window_handle(&self) -> Result<RawWindowHandle, HandleError> {
        if self.open.get() {
            ...
            return Ok(RawWindowHandle::AppKit(handler));
        }
...
impl<'a> Window<'a> {
        ...
        B: Send + 'static,
    {
        ...
        let handle = if let Ok(handle) = parent.raw_window_handle() {
    ...
    fn create_gl_context(ns_window: Option<id>, ns_view: id, config: GlConfig) -> GlContext {
        ...
        let handle = if let Some(ns_view) = ns_view {
            ...
            panic!("NSView can not be null!")
        };

        unsafe {
            GlContext::create(&RawWindowHandle::AppKit(handle), config)
...
impl<'a> HasWindowHandle for Window<'a> {
    fn window_handle(&self) -> Result<raw_window_handle::WindowHandle<'_>, HandleError> {
        let raw_handle = if let Ok(raw_handle) = self.inner.raw_window_handle() {
            raw_handle
        } else {
            return Err(HandleError::Unavailable);
        };
